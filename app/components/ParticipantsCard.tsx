...
export function ParticipantCounter() {
  ...
  const [eventId, setEventId] = useState<string | null>(null); // Added state to track the specific GameEvent ID
  ...
  const loadParticipants = useCallback(async () => {
    ...
    try {
      ...
      setHasJoined(hasUserJoined(data.participants, CURRENT_USER));
  ...
  const getTimeUntilReset = (): string => {
    ...
    // Updated to show minutes even when there is more than 1 day left!
  ...
  const handleToggle = async () => {
    ...
    try {
      if (hasJoined) {
        // Leave event
        await leaveWeeklyEvent(eventId, CURRENT_USER);

        // Optimistic UI Update
        setParticipants(prev => prev.filter(p => p.userId !== CURRENT_USER));
        ...
        // Join event
        await joinWeeklyEvent(eventId, CURRENT_USER);

        // Optimistic UI Update
        setParticipants(prev => [...prev, { userId: CURRENT_USER, userName: CURRENT_USER }]);
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <button
          ...
          className={`w-full md:w-auto px-6 py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 disabled:cursor-not-allowed ${hasJoined
          ...
          <div className="mt-6 pt-4 border-t border-blue-400/20">
            ...
            <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
              {participants.map((participant, idx) => (
                <span
                  key={idx}
                  className={`text-xs px-2 py-1 rounded-full ${participant.userId === CURRENT_USER
